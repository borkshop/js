...
const testRandom = test.macro({
  ...
  title(providedTitle = '', seed) {
    return `${providedTitle} random(${seed instanceof ArrayBuffer
...
const testRandomInt = test.macro({
  ...
  title(providedTitle = '', seed) {
    return `${providedTitle} randomInt(${seed instanceof ArrayBuffer
      ? [...new Uint8Array(seed)].map(n => n.toString(16).padStart(2, '0')).join(' ')
      : typeof seed == 'string'
        ? JSON.stringify(seed)
        : seed.toString(16)
...
const testRandomBigint = test.macro({
  ...
  title(providedTitle = '', seed) {
    return `${providedTitle} randomBigint(${seed instanceof ArrayBuffer
      ? [...new Uint8Array(seed)].map(n => n.toString(16).padStart(2, '0')).join(' ')
      : typeof seed == 'string'
        ? JSON.stringify(seed)
        : seed.toString(16)
...
const testRandomBytes = test.macro({
  ...
  exec(t, seed, bufSize) {
    ...
    // NOTE: this test has a wide margin of error ( up to 40% variation per
  ...
  title(providedTitle = '', seed, bufSize) {
    return `${providedTitle} randomBytes(${seed instanceof ArrayBuffer
      ? [...new Uint8Array(seed)].map(n => n.toString(16).padStart(2, '0')).join(' ')
      : typeof seed == 'string'
        ? JSON.stringify(seed)
        : seed.toString(16)
